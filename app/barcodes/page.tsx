...
export default function ColoursTable() {
    ...
    const filteredData = React.useMemo(() => {
        ...
        return data.filter(
            (item) =>
                item.cutting_barcode.toLowerCase().includes(debouncedSearch.toLowerCase())
        );
    }, [data, debouncedSearch]);

    const columns: ColumnDef<ColorInfo>[] = [
        {
            accessorKey: "cutting_roll_id",
            header: "ID",
        },
        {
            accessorKey: "cutting_barcode",
            header: "Colour Name",
        },
    ];
                ...
                <header className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-[[data-collapsible=icon]]/sidebar-wrapper:h-12">
                    <div className="flex items-center gap-2 px-4">
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                <BreadcrumbItem>
                                    <BreadcrumbPage className="text-2xl font-bold">Barcodes Test</BreadcrumbPage>
                                </BreadcrumbItem>
                            </BreadcrumbList>
                        </Breadcrumb>
                    </div>
