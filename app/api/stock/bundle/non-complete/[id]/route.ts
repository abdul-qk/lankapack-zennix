...
export async function GET(
  ...
  try {
    ...
    // Fetch non-complete items related to this bundle
    const items = await prisma.hps_non_complete_item.findMany({
      where: {
        non_complete_info: bundleId,
    ...
    return NextResponse.json(
      { error: "Failed to fetch non-complete items" },
      { status: 500 }
    );
  }
}

// DELETE: Delete a non-complete item (soft delete by updating del_ind to 0)
export async function DELETE(
  ...
  try {
    ...
    const item = await prisma.hps_non_complete_item.findUnique({
      where: {
        non_complete_id: id,
      },
    });
