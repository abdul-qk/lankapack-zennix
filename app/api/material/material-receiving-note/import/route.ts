    ...
    try {
        ...
        for (const item of itemsData) {
            ...
            const itemDataForDb = {
                ...
                material_colour: String(item.material_colour),
        ...
        const newMaterialInfo = await prisma.hps_material_info.create({
            data: {
                ...
                total_reels: itemsToCreate.length, // Use count of items actually being created
                total_net_weight: totalNetWeight,
                total_gross_weight: totalGrossWeight,
                add_date: new Date(),
                user_id: 1, // TODO: Get actual user ID
                material_info_status: 1, // Default status
                material_items: {
                    create: itemsToCreate // Use the cleaned data for creation
                }
            },
            include: {
                material_items: true, // Include created items in the response if needed
            },
        });

        return NextResponse.json({ message: `Successfully imported ${itemsToCreate.length} items.`, data: newMaterialInfo }, { status: 201 });
