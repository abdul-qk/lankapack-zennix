...
export default function ReturnTable() {
    const [data, setData] = React.useState<ReturnInfo[]>([]);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            toast({
                variant: "destructive",
                title: "Error",
                description: "Failed to fetch return data"
            });
        }
        setLoading(false);
    };
    ...
    const handleDelete = async (id: number) => {
        try {
            const response = await fetch(`/api/sales/return?id=${id}`, {
                method: 'DELETE',
            });
            ...
            toast({
                variant: "destructive",
                title: "Error",
                description: "Failed to delete return record"
            });
        }
    ...
    const filteredData = React.useMemo(() => {
        ...
        return data.filter(
            (item) =>
                ...
                item.return_no_bags.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
                item.return_info_id.toString().includes(debouncedSearch)
        );
    }, [data, debouncedSearch]);

    const columns: ColumnDef<ReturnInfo>[] = [
        {
            accessorKey: "return_info_id",
            header: "ID",
        },
        {
                ...
                <header className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-[[data-collapsible=icon]]/sidebar-wrapper:h-12">
                    <div className="flex items-center gap-2 px-4">
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                <BreadcrumbItem>
                                    <BreadcrumbPage className="text-2xl font-bold">Return Records</BreadcrumbPage>
                                </BreadcrumbItem>
                            </BreadcrumbList>
                        </Breadcrumb>
                    </div>
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This action will delete the return record. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
