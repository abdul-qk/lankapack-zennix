...
type ReturnInfo = {
  return_info_id: number;
  customer_name: number;
  customer_address: string;
  customer_contact: string;
  return_no_bags: string;
  add_date: string;
  user_id: number;
  del_ind: number;
};
...
export default function ReturnView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`/api/sales/return/${params.id}`);
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Return Note #${data?.returnInfo.return_info_id}</title>
          <style>
            @page {
              size: A4;
              margin: 1cm;
  ...
  const { returnInfo, customer, returnItems, totalAmount } = data;
  const formattedDate = returnInfo.add_date
    ? format(new Date(returnInfo.add_date), "dd/MM/yyyy")
    : "N/A";
        ...
        <header className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-[[data-collapsible=icon]]/sidebar-wrapper:h-12">
          <div className="flex items-center gap-2 px-4">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                <BreadcrumbItem>
                  <BreadcrumbPage className="text-2xl font-bold">View Return #{returnInfo.return_info_id}</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
            ...
            <div ref={printRef} className="a4-page">
              ...
              <div className="print-header text-center mb-8">
                ...
                <h1 className="text-xl font-bold mt-4">RETURN NOTE</h1>
              </div>
              ...
              <div className="customer-info grid grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <p><strong>Contact:</strong> {customer.customer_mobile || returnInfo.customer_contact}</p>
                </div>
                <div className="text-right">
                  <p><strong>Return ID:</strong> {returnInfo.return_info_id}</p>
                  <p><strong>Date:</strong> {formattedDate}</p>
                  <p><strong>Total Bags:</strong> {returnInfo.return_no_bags}</p>
                </div>
              </div>
