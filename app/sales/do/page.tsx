...
export default function DeliveryOrderTable() {
    const [data, setData] = React.useState<SalesInfo[]>([]);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/sales/do`);
            ...
            toast({
                variant: "destructive",
                title: "Error",
                description: "Failed to fetch delivery order data"
            });
        }
        setLoading(false);
    };
    ...
    const handleDelete = async (id: number) => {
        try {
            const response = await fetch(`/api/sales/do?id=${id}`, {
                method: 'DELETE',
            });
    ...
    const filteredData = React.useMemo(() => {
        ...
        return data.filter(
            (item) =>
                ...
                item.sales_no_bags.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
                item.sales_info_id.toString().includes(debouncedSearch)
        );
    }, [data, debouncedSearch]);

    const columns: ColumnDef<SalesInfo>[] = [
        {
            accessorKey: "sales_info_id",
            header: "ID",
        },
        {
                ...
                <header className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-[[data-collapsible=icon]]/sidebar-wrapper:h-12">
                    <div className="flex items-center gap-2 px-4">
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                <BreadcrumbItem>
                                    <BreadcrumbPage className="text-2xl font-bold">Delivery Orders</BreadcrumbPage>
                                </BreadcrumbItem>
                            </BreadcrumbList>
                        </Breadcrumb>
                    </div>
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This action will delete the delivery order. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
