...
import { Pencil, PlusCircle, EyeIcon, ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight } from "lucide-react";
...
export default function BundlesTable() {
    const [data, setData] = React.useState<BundleInfo[]>([]);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/stock/bundle`);
    ...
    const columns: ColumnDef<BundleInfo>[] = [
        {
            accessorKey: "bundle_info_id",
            header: "ID",
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex space-x-2">
                        <Link href={`/stock/bundles/view/${item.bundle_info_id}`}>
                            <Button variant={"outline"} size="sm">
                                <EyeIcon size={16} />
                            </Button>
                        </Link>
                        <Link href={`/stock/bundles/edit/${item.bundle_info_id}`}>
                            <Button size="sm">
                                <Pencil size={16} />
                            </Button>
                        </Link>
                ...
                <header className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-[[data-collapsible=icon]]/sidebar-wrapper:h-12">
                    <div className="flex items-center gap-2 px-4">
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                <BreadcrumbItem>
                                    <BreadcrumbPage className="text-2xl font-bold">Bundles/Pack</BreadcrumbPage>
                                </BreadcrumbItem>
                            </BreadcrumbList>
                        </Breadcrumb>
                    </div>
